...
ts.plot(data1, xlab="Periode", ylab="Harga", main= "SMA N=3")
points(data1)
...
MAPE_train.sma = mean(abs((error_train.sma[4:length(train_ma.ts)]/train_ma.ts[5:length(train_ma.ts)])*100))
...
##7. metode SES
```{r}
ses.1 <- ses(train_ma.ts, h = 27, alpha = 0.1)
plot(ses.1)
ses.1
```
```{r}
...
```{r}
#(fungsi Holtwinter)
ses1<- HoltWinters(train_ma.ts, gamma = FALSE, beta = FALSE, alpha = 0.1)
plot(ses1)
